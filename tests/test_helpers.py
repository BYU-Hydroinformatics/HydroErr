...
class TestHelperFunctions:
    ...
    def test_treat_values_remove(self):
        ...
        check.is_true("Row(s) [0 1] contained NaN values and the row(s) have been removed "
                        "(Rows are zero indexed)." in str(w[0].message))
        check.is_true("Row(s) [2 3] contained Inf or -Inf values and the row(s) have been "
                        "removed (Rows are zero indexed)." in str(w[1].message))
        check.is_true("Row(s) [4 5] contained zero values and the row(s) have been removed "
                        "(Rows are zero indexed)." in str(w[2].message))
        check.is_true("Row(s) [6 7] contained negative values and the row(s) have been "
                        "removed (Rows are zero indexed)." in str(w[3].message))

        check.is_none(np.testing.assert_equal(sim_treated, a[8:, 0]),
                          "Treat values function did not work properly when removing values from "
                          "the simulated data.")
        check.is_none(np.testing.assert_equal(obs_treated, a[8:, 1]),
                          "Treat values function did not work properly when removing values from "
                          "the observed data.")
    ...
    def test_treat_values_replace(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            check.is_true(issubclass(w[0].category, UserWarning))
            check.is_true(issubclass(w[1].category, UserWarning))
            check.is_true("Elements(s) [0] contained NaN values in the simulated array and "
            ...
            check.is_true("Elements(s) [1] contained Inf values in the simulated array and "
            ...
            check.is_none(np.testing.assert_equal(sim_treated, sim_new),
                              "Treat values function did not work properly when replacing values from "
                              "the simulated data.")
            check.is_none(np.testing.assert_equal(obs_treated, obs_new),
                              "Treat values function did not work properly when replacing values from "
                              "the observed data.")
    ...
    def test_treat_values_unequal_length(self):
        ...
        check.is_true("The two ndarrays are not the same size." in context.value.args[0])
